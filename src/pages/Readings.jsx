...
export default function Readings({ session }) {
  ...
  const s = {
    ...
    body: { maxWidth: '1000px', margin: '0 auto', padding: '3rem 1.5rem 5rem' },
    header: { marginBottom: '2.5rem' },
    title: { fontFamily: "'Playfair Display',Georgia,serif", fontSize: 'clamp(1.6rem,4vw,2.2rem)', color: '#e8d5b0', marginBottom: '0.3rem' },
    sub: { color: '#5a5248', fontSize: '0.85rem' },
    layout: { display: 'grid', gridTemplateColumns: selected ? '320px 1fr' : '1fr', gap: '1.5rem', alignItems: 'start' },
    ...
    reading: { background: 'rgba(200,169,126,0.025)', border: '1px solid rgba(200,169,126,0.1)', borderRadius: '14px', padding: '2rem' },
    ...
    empty: { textAlign: 'center', padding: '4rem 2rem', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px' },
    ...
    <div style={s.page}>
      <nav style={s.nav}>
        ...
        <div style={{ display: 'flex', gap: '0.7rem' }}>
          <button style={s.navBtn} onClick={() => navigate('/session')}>New Session</button>
              ...
              {sessions.map(sess => (
                <div key={sess.id} style={s.card(selected?.id === sess.id)} onClick={() => setSelected(sess)}
                  ...
                  <div style={s.cardFooter}>
                    ...
                    <button style={s.deleteBtn} onClick={e => { e.stopPropagation(); handleDelete(sess.id) }} disabled={deleting}>
                      Delete
                    </button>
                  </div>
                </div>
